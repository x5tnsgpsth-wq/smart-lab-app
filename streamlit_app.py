...
# 1. إعدادات الصفحة - وضع static لزيادة الثبات
st.set_page_config(page_title="Lab System v32", page_icon="🔬", layout="wide")

# 2. إدارة البيانات (تحسين سرعة الاستجابة)
...
# 3. كود منع إعادة التحميل (Anti-Refresh & Smooth Scrolling)
st.markdown(f"""
    <style>
    /* منع إعادة التحميل عند السحب لأسفل في الموبايل والتابلت */
    body {{
        overscroll-behavior-y: contain;
    }}
    
    /* تثبيت رأس الصفحة ومنع اهتزاز المحتوى */
    .main {{
        overflow: auto;
    }}
    ...
    /* إخفاء شريط التحميل المزعج في الأعلى */
    #stDecoration {{
        display:none;
    }}
...
# استخدام التبويبات مع حاوية ثابتة
tabs = st.tabs(["📝 الإدخال", "📄 التقرير", "📊 الإحصائيات", "⚙️ الإعدادات"])

# --- التبويب 4: الإعدادات (تحديث بدون إعادة تحميل المتصفح) ---
with tabs[3]:
...
# --- التبويب 1: إدخال البيانات (ثبات الحقول) ---
with tabs[0]:
    NR = get_nr()
...
# --- التبويب 3: الإحصائيات (تحميل خفيف) ---
with tabs[2]:
    if not st.session_state.df.empty:
